...
{
    class Cooling : TemperatureAverage
    {
        public Cooling()
        {
        ...
        private void WriteSetValuesTORun()
        {
            ...
            {
                long Charge_Id = (long)temp.Rows[0]["Id"];
                string CZTemp = Math.Round((float)ApplicationService.GetVariableValue("NLM4.PLC.Blocks.4 Modul 4.08 Heizung / Ventilatoren.04 Kühlzone.DB Kühlzone HMI.Sollwerte.Temperatur"), 1).ToString();
        ...
        private void WriteActualValuesTORun()
        {
            ...
            DataTable temp = (new LocalDBAdapter("SELECT Id " +
                                                 "FROM Charges " +
                                                 "WHERE Order_Id = " + VWV_Order_Id_E.Value + " AND Charge = " + VWV_Charge_E.Value + ";")).DB_Output();
            double[] gavCZ = GetActualValues();
            double CZTempMin = gavCZ[0];
            double CZTemp = gavCZ[1];
            double CZTempMax = gavCZ[2];
            if (temp.Rows.Count > 0)
            {
                long Charge_Id = (long)temp.Rows[0]["Id"];
                ...
                if (temp.Rows.Count > 0)
                {
                    ...
                    var b = (new LocalDBAdapter("UPDATE ActualValues " +
                                                "SET CZTemp = '" + CZTemp + "', CZTempMin = '" + CZTempMin + "', CZTempMax = '" + CZTempMax + "' " +
                                                "WHERE Id = " + ActualValues_Id + ";")).DB_Input();
                }
            }
            VS.SetValue(VN_AverageTemperature + "[" + VWV_Run_E.Value + "]", CZTemp);
        ...
        private void WriteErrors()
        {
            ...
            if (TT.Length != 0)
                if (!Enumerable.SequenceEqual(TT, OLD_Error))
                {
                    for (int i = 3; i <= 12; i++)
                    {
                        if((bool)VS.GetValue("NLM4.PLC.Blocks.7 Tracking / Kommunikation.DB Tracking Kühlzone.Tablett["+i+"].Status.Charge.Material vorhanden",true))
