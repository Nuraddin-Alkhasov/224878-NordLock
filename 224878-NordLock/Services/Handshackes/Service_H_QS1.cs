...
{
    [ExportService(typeof(IQS1))] 
    [Export(typeof(IQS1))]
    public class Service_H_QS1 : ServiceBase, IQS1
    {
        IVariableService VS;
        ...
        void W1_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            try
            {
                uint OrderId = (uint)ApplicationService.GetVariableValue("NLM4.PLC.Blocks.50 HMI.01 PC.DB PC.QS 1.Handshake.to PC.Order Id");
                ...
                if (DT.Rows.Count > 0)
                {
                    ApplicationService.SetVariableValue("NLM4.PLC.Blocks.50 HMI.01 PC.DB PC.QS 1.Handshake.Data.Quality Order#STRING12", DT.Rows[0]["Data_1"]);
                    ApplicationService.SetVariableValue("NLM4.PLC.Blocks.50 HMI.01 PC.DB PC.QS 1.Handshake.Data.Quality Batch#STRING12", DT.Rows[0]["Data_2"]);
                    ApplicationService.SetVariableValue("NLM4.PLC.Blocks.50 HMI.01 PC.DB PC.QS 1.Handshake.Data.Quality Item#STRING12", DT.Rows[0]["Data_3"]);

                    ApplicationService.SetVariableValue("NLM4.PLC.Blocks.50 HMI.01 PC.DB PC.QS 1.Handshake.from PC.Loaded", true);
                    return;
                }
                else 
                {
                     ApplicationService.SetVariableValue("NLM4.PLC.Blocks.50 HMI.01 PC.DB PC.QS 1.Handshake.from PC.Not loaded", true); 
                }
            }
            catch 
            {
        ...
        protected override void OnLoadProjectCompleted()
        {
            ...
            NLDataToPLC = VS.GetVariable("NLM4.PLC.Blocks.50 HMI.01 PC.DB PC.QS 1.Handshake.to PC.GetData");
